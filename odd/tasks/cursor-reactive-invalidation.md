...
- [x] `run-supervisor.js`: calls the hook per real line (both streams) via `handoff.model`, invalidates at most once per pool per run (a per-run `Set` of dedupe keys), best-effort — awaited inside the same `processing` chain the "close" handler drains, error absorbed (`.catch(() => {})`), so a failure never alters the real run's own outcome and the run can never complete before its own invalidation has settled.
- [x] Tests: 6 new in `cursor-entitlement.test.js` (`detectCursorLimitFromOutput` — JSON is_error+limit, stderr limit, normal assistant text ignored, unrecognized is_error ignored, non-JSON/empty ignored, ordinary stderr noise ignored); 4 new in `cursor-entitlement-store.test.js` (`invalidateStoredCursorPoolAccess` — real disk round-trip + pool independence, no-op on nothing cached, no-op on pool not cached, best-effort on read/write failure); new `cursor-execution-adapter.test.js` (5 tests — real pool derivation + invalidate closure for both pools, `auto` exemption, no-model/non-matching returns null, another adapter's real no-op default); new `run-supervisor-cursor-invalidation.test.js` (5 tests, full in-process `startRun`→`supervisePreparedRun` path — real limit hit invalidates and the run still completes, ordinary output never invalidates, 3 repeated matching lines invalidate exactly once, a slow invalidate settles before the run completes, a failing invalidate never alters the run's own outcome).
- [x] Full suite green: 2034/2034 (2014 + 20 new), zero real-disk leakage.
- [x] Post-review fix (2026-09-22, commit `4506679` amended by a follow-up commit): a blind adversarial review caught a real ordering race — `handleLine`'s `Promise.resolve(quotaHit.invalidate(homeDir)).catch(() => {})` was fire-and-forget, not chained into the `processing` queue the "close" handler awaits, so a slow invalidate could still be in flight when the run reported COMPLETED. "Best-effort" was supposed to mean "awaited, errors absorbed," not "detached." Fixed by `await`-ing the call in place (still `.catch(() => {})`'d, so a real failure still can't alter the run's own outcome) — now provably part of the same chain via a dedicated regression test with a controlled, delayed `invalidate()` (RED before the fix, GREEN after). Real changed-line count for the full local commit: **490** (over the ~400 planning heuristic) — the review round, its regression test, and this fix account for the overage; the review found no other defect.
