...
- [x] Shipped as v0.35.1: PR #325 merged to main with a real merge commit (e00f8c7), release commit 92d6e65, tagged `kairo-runtime-v0.35.1`, published to npm (CI green on Node 20/22/24 — one Node 20 run hit a known, pre-existing, unrelated `quick-ask.test.js` timing flake under full-suite CI load, documented in that test's own comment; a clean re-run of just that job confirmed it, not a real regression), global install verified (`kairo --version` → 0.35.1, gitHead matches 92d6e65).
...
- Scope 4 (PROJECT TEAM readable detail block) complete: `af9b01e` (initial readable detail block) + `336a6a5` (pre-ship correction — strictly role+model rows, click-selects/Enter-edits), shipped as v0.35.1: PR #325 merged to main (e00f8c7), release commit 92d6e65, tagged `kairo-runtime-v0.35.1`, published to npm, global install verified (`kairo --version` → 0.35.1, gitHead matches 92d6e65): 2026-09-22
- cursor-access-auto-detection is now fully closed: all 4 scopes shipped (v0.35.0 for Scopes 1-3, v0.35.1 for Scope 4). Only remaining open item is the deferred, non-blocking `invalidateCursorPoolAccess` real-execution wiring noted above.
